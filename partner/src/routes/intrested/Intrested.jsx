...
const Intrested = () => {
  ...
  const handlePostResponse = (postResponse) => {
    ...
    if (postData.length > 0) { 
      return postData.map((post) => <Card intrested={true} item={post} key={post.postId} />);
    } else {
      setTotalPost(0);
      setIsLoadingMore(false);
      return (
    ...
    <div className="Intrested pt-4">
      <div className="cards mb-5">
        ...
        >
          ...
          <button 
            disabled={totalPost > 0}
            ref={loadMoreButtonRef}
            className={`btn text-light shadow-none mt-3 d-flex fs-5 justify-content-center mx-auto `}
            onClick={loadMore}
          >
            {isLoadingMore ? (

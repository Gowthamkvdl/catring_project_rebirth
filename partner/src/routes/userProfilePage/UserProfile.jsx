...
const ProfilePage = () => {
  ...
  // Fetch user data based on their category (cater or server)
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        let fetchUser;

        // Try fetching user as a 'Cater' first
        try {
          fetchUser = await apiRequest.get(`/user/oneCater/${userId}`);
          if (fetchUser?.data) {
            setUser(fetchUser.data);
            setCategory("cater");
            return; // Exit if 'Cater' user is found
          }
        } catch (error) {
          console.log("Cater user not found, trying Server next...");
        }

        // If no 'Cater' user found, try fetching user as a 'Server'
        try {
          const fetchServerUser = await apiRequest.get(
            `/user/oneServer/${userId}`
          );
          if (fetchServerUser?.data) {
            setUser(fetchServerUser.data);
            setCategory("server");
            return; // Exit if 'Server' user is found
          } else {
  ...
  // Fetch user events based on their category (server or cater)
  useEffect(() => {
    const showMyEvents = async () => {
      try {
        setMyEventsLoading(true);
        const eventsResponse = await apiRequest.get(
          `/user/${category === "server" ? "oneServer" : "oneCater"}/${userId}`
        );

        const myEvents =
          category === "server"
            ? eventsResponse.data.savedPosts
            : eventsResponse.data.post;
  ...
  const submitRating = async () => {
    try {
      await apiRequest.post("/user/rating", {
        profileId: userId,
        starCount: rating,
      });
      toast.success(
          ...
          <div className="row my-3">
            <div className="col-12">
              ...
              <div className="remarks">
                <div className="subtitle-text mt-3">Remarks:</div>
                <div className="body-text mt-1">{user.remarks? user.remarks : "No remarks"}</div>
              </div>
            </div>
          </div>
        </div>
        {category === "cater" && (
          <div className="col-12 col-md-6 mb-4 mt-2 p-0 m-0 px-md-4">
            <h4 className="text-uppercase">
              {user?.name ? user?.name : "USER"}'s{" "}
              {category === "server" ? "Saved" : ""} Events
            </h4>
            {(myEventsLoading || savedEventsLoading) && (
              <Loader message={"Loading..."} />
            )}
            {!(myEventsLoading || savedEventsLoading) && events.length > 0 ? (
              events
                .slice()
                .reverse()

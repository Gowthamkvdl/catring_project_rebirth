...
const SinglePage = () => {
  ...
  const [saved, setSaved] = useState(post.postData.isSaved);
  const [postStatus, setPostStatus] = useState(post.postData.disabled);
          ...
          <div className="row">
            <div className="col-12 ">
              ...
              <h4 className="mt-1 fs-1 row">
                <div className="small-text opacity-75 mt-3">Event Name</div>
      ...
      <div className="mt-3 mx-1 mb-2 ">
        <div className="mt-3 mb-2  d-flex gap-2 flex-wrap">
          <div className="body-text">
            Total interested servers: {post.servers && post.servers.length}
          </div>
          <div className="body-text">
            Total accepted servers:{" "}
            {post.servers &&
              post.servers.filter((server) => server.status === "accepted")
                .length}
          </div>
          <div className="body-text mb-2">
            Total rejected servers:{" "}
            {post.servers &&
              post.servers.filter((server) => server.status === "rejected")
                .length}
          </div>
        </div>
        {post.servers && post.servers.length > 0 ? (
          post.servers.map((server, index) => (
            <ProfileCard
              key={index}
